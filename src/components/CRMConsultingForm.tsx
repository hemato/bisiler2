    ...
    return (
      <div className="text-center py-12 bg-green-50 rounded-2xl border border-green-200">
        ...
        <p className="text-secondary-600 mb-6">
          {lang === 'en'
            ? 'Your CRM consulting request has been received. Our expert team will contact you within 24 hours.'
            : 'CRM danışmanlığı talebiniz alınmıştır. Uzman ekibimiz 24 saat içerisinde sizinle iletişime geçecektir.'
          }
        </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            >
              ...
              {translations.crmConsulting.form.options.employeeCount.map((option: string) => (
          ...
          <div>
            ...
            >
              <option value="">{lang === 'en' ? 'Select...' : 'Seçiniz...'}</option>
              {translations.crmConsulting.form.options.currentSystem.map((option: string) => (
                <option key={option} value={option}>{option}</option>
              ))}
            </select>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            >
              <option value="">{lang === 'en' ? 'Select...' : 'Seçiniz...'}</option>
              {translations.crmConsulting.form.options.mainChallenge.map((option: string) => (
                <option key={option} value={option}>{option}</option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            >
              <option value="">{lang === 'en' ? 'Select...' : 'Seçiniz...'}</option>
              {translations.crmConsulting.form.options.preferredCrm.map((option: string) => (
                <option key={option} value={option}>{option}</option>
              ))}
            </select>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            >
              <option value="">{lang === 'en' ? 'Select...' : 'Seçiniz...'}</option>
              {translations.crmConsulting.form.options.budgetRange.map((option: string) => (
                <option key={option} value={option}>{option}</option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            >
              <option value="">{lang === 'en' ? 'Select...' : 'Seçiniz...'}</option>
              {translations.crmConsulting.form.options.timeline.map((option: string) => (
                <option key={option} value={option}>{option}</option>
              ))}
            </select>
          </div>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={handleChange}
            className="w-full px-4 py-3 border border-secondary-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors"
            placeholder={lang === 'en' ? 'Tell us about your CRM needs...' : 'CRM ihtiyaçlarınızı anlatın...'}
          />
        </div>
